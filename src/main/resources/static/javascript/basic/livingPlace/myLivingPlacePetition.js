...
function mapearPropuestas(propuesta){
    return `<tr>
                ...
                <td>${propuesta.propietario}</td>
                <td>${propuesta.tipo}</td>
                <td>${propuesta.precio}</td>
                <td>
                    <button type="button" class="btn btn-primary" onclick="eliminarPropuesta('${propuesta.direccion}')">
...
function eliminarPropuesta(direccion) {
    fetch('http://localhost:8080/api/livingPlace/deleteRequest/' + direccion, {
        method: 'DELETE',
    })
    .then(response => {
        if (response.ok) {
