...
function mapearEmpresa(empresa){
    return `<tr>
        ...
        <td>
            <button type="button" class="btn btn-primary" onclick="eliminarEmpresa('${empresa.cif}')">
...
function eliminarEmpresa(cifEmpresa) {
    fetch('http://localhost:8080/api/company/delete/' + cifEmpresa, {
        method: 'DELETE',
    })
    .then(response => {
        if (response.ok) {
