...
function mostrarUsuarios(){
    fetch("http://localhost:8080/api/user/getAllDown").then(res => {
        res.json().then(json => {
            users = json;
            imprimirUsuarios();
        });
...
function mapearUsuario(usuario){
    return `<tr>
            ...
            <td>
                <button type="button" class="btn btn-primary" onclick="eliminarUsuario('${usuario.dni}')">
...
function eliminarUsuario(dni) {
    fetch('http://localhost:8080/api/user/delete/' + dni, {
        method: 'DELETE',
    })
    .then(response => {
        if (response.ok) {
